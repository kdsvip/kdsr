...
def MainMenu():
	addDir('תפריט בילדים','',1,addon_icon,addon_fanart,local_string(30001),isFolder=True)
	addDir('תחזוקה','',5,addon_icon,addon_fanart,local_string(30002),isFolder=True)
	addDir('מחיקה','',4,addon_icon,addon_fanart,local_string(30003),isFolder=False)
	addDir('עדכונים','',100,addon_icon,addon_fanart,'Bring up the notifications dialog',isFolder=False)
...
def main(NAME, NAME2, VERSION, URL, ICON, FANART, DESCRIPTION):
	...
	yesInstall = dialog.yesno(NAME, 'ויזארד ההתקנה מוכן להתקנת הבילד', nolabel='ביטול', yeslabel='המשך')
	if yesInstall:
	    save_check()
	    save_move1()
	    yesFresh = dialog.yesno('התקנה נקיה', 'האם לנקות את הקודי שלך לפני התקנת הבילד?', nolabel='לא', yeslabel='התקנה נקיה')
	    if yesFresh:
	    	freshStart()
...
def freshStart():
	yesFresh = dialog.yesno('מחיקה', 'אנא אשר המשך מחיקת נתוני הקודי שלך', nolabel='לא', yeslabel='המשך')
	if yesFresh:
		
		#Skin Switch
		if not currSkin() in ['skin.confluence', 'skin.estuary', 'skin.estouchy', 'skin.Phenomenal']:
		...
		dp.create(addon_name, 'מוחק קבצים ותיקיות...')
		xbmc.sleep(1000)
		dp.update(30, 'מוחק קבצים ותיקיות...')
		xbmc.sleep(1000)
		...
		dp.update(60, 'מוחק קבצים ותיקיות...')
		xbmc.sleep(1000)	
		for root, dirs, files in os.walk(xbmcPath,topdown=True):
			dirs[:] = [d for d in dirs if d not in EXCLUDES]
			for name in dirs:
		...
		if not os.path.exists(packages):
			os.mkdir(packages)
		dp.update(100, 'מוחק קבצים ותיקיות...בוצע')
		xbmc.sleep(2000)
		if mode == 4:
			save_move2()
			...
			dialog.ok(addon_name, 'התהליך הסתיים לחץ לאישור לסגירת הקודי')
			os._exit(1)
	else:
		return
...
def buildInstall(NAME, NAME2, VERSION, URL):
	...
	dp.create(NAME + ' - ' + str(length2) + ' MB', 'מוריד את הבילד...אנא המתן')
	dp.update(0, 'מוריד את הבילד...אנא המתן')
	#
	if length:
		...
		while True:
			...
			tempzip.write(buf)
			dp.update(size2, 'מוריד את הבילד...אנא המתן' + str(size2) + '/' + str(length2) + 'MB')
				
	else:
		dp.update(50, 'מוריד את הבילד...אנא המתן')
		tempzip.write(zipresp.read())
	if length:
		dp.update(100, 'מוריד את הבילד...אנא המתן' + str(size2) + '/' + str(length2) + 'MB...הסתיים!')
	else:
		dp.update(100, 'מוריד את הבילד...הסתיים!')
	xbmc.sleep(1000)      
	tempzip.close()
	dp.update(66, 'פורס קבצי הבילד...אנא המתן')
	xbmc.sleep(1000)
	zf = ZipFile(zippath)
	zf.extractall(path = home)
	dp.update(100, 'פורס קבצי הבילד...הסתיים!')
	xbmc.sleep(2000)
	zf.close()
	os.unlink(zippath)
	save_move2()
	...
	addon.setSetting('firstrun', 'true')
	dialog.ok(addon_name, 'ההתקנה הסתיימה, לחץ אישור לסגירת הקודי')
	os._exit(1)
