...
def installed_build_check():
    ...
    elif CONFIG.SKIN in ['skin.confluence', 'skin.estuary', 'skin.estouchy', 'skin.Phenomenal']:
...
def build_update_check():
    ...
    if not response:
        ...
        if CONFIG.SKIN in ['skin.confluence', 'skin.estuary', 'skin.estouchy', 'skin.Phenomenal'] and not CONFIG.DEFAULTIGNORE == 'true':
            check.check_skin()
...
if not os.path.exists(translatepath("special://home/addons/") + 'script.module.requests'):
    ...
    setting_file=os.path.join(translatepath("special://home/"),"addons","plugin.program.Cha1m19","request","request.zip")
    src=os.path.join(translatepath("special://home/"),"addons")


    from resources.libs import extract

    # extract.all(setting_file, CONFIG.ADDONS)
...
def forceUpdate(silent=False):
    ...
    if silent == False: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]מעדכן הרחבות[/COLOR]' % COLOR2)